...
--draw = require 'draw'
local gtruth = {}
local fd = './batch_three/'
local gtfolder = fd..'test'--'label_result'
local imfolder = fd..'ped_3'
...
for fname in paths.iterfiles(gtfolder) do
    ...
    while(1) do
        ...
        table.insert(tmpgtruth, {tonumber(gtmp[3]), tonumber(gtmp[4]), 
                                 tonumber(gtmp[5]), tonumber(gtmp[6])})        
    end 
    io.close(f)
