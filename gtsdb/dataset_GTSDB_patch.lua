...
GTFILE = '/media/administrator/文档/我的文件/Tencent_Model/detection_model_small/resized/gt.txt'
IMREFILE = '/media/administrator/文档/我的文件/Tencent_Model/detection_model_small/resized/'
  ...
  for indr = 1, 480, step do
  	...
  	for indc = 1, 640, step do
  	  ...
  	  -- Load image mask 		
  		...
  		-- Select data
  		if (score == 0) then--if (score >= 0.75) then --
            ...
            -- Generate bbox
						IMAGEMASK[{ indpatch,2,{top_s,bottom_s},{left_s,right_s} }] = top_s / masksize
						IMAGEMASK[{ indpatch,3,{top_s,bottom_s},{left_s,right_s} }] = bottom_s / masksize
						IMAGEMASK[{ indpatch,4,{top_s,bottom_s},{left_s,right_s} }] = left_s / masksize
						IMAGEMASK[{ indpatch,5,{top_s,bottom_s},{left_s,right_s} }] = right_s / masksize					
...
for i = 1, mask:size(2) do
	for j = 1, mask:size(3) do
		if (mask[{ 1,i,j }] >= 0.95) then
			local cen_r = (i-1)*4+1
		  local cen_c = (j-1)*4+1 
			local top = torch.round(cen_r-IMAGEMASK[{ ind,2,i,j }]*patchsize)
			local bottom = torch.round(cen_r+IMAGEMASK[{ ind,3,i,j }]*patchsize)
			local left = torch.round(cen_c-IMAGEMASK[{ ind,4,i,j }]*patchsize)
			local right = torch.round(cen_c+IMAGEMASK[{ ind,5,i,j }]*patchsize)
			print(IMAGEMASK[{ ind,2,i,j }])
			print(IMAGEMASK[{ ind,3,i,j }])
			print(IMAGEMASK[{ ind,4,i,j }])
			print(IMAGEMASK[{ ind,5,i,j }])
