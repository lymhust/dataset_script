...
local MASK_BOX = torch.Tensor(totalnum, 5, IMH/4, IMW/4):zero()
...
for i = 1, totalnum do
  ...
  for k = 1, #tmpgruth[2] do
    ...
    -- Mask
    MASK_BOX[{ i,1,{top_s,bottom_s},{left_s,right_s} }] = 1
    -- BBox
    MASK_BOX[{ i,2,{top_s,bottom_s},{left_s,right_s} }] = left/IMW    -- Left
    MASK_BOX[{ i,3,{top_s,bottom_s},{left_s,right_s} }] = right/IMW   -- Right
    MASK_BOX[{ i,4,{top_s,bottom_s},{left_s,right_s} }] = top/IMH     -- Top
    MASK_BOX[{ i,5,{top_s,bottom_s},{left_s,right_s} }] = bottom/IMH  -- Bottom
    --image.display(MASK_BOX[i])
  end
	print(i)
end
...
local data = {IMAGE, MASK_BOX}
torch.save('./data/DATA.t7', data)
...
local ind = 2
local img = IMAGE[{ ind,{},{},{} }]
local mask = MASK_BOX[{ ind,{1},{},{} }]
for i = 1, mask:size(2) do
	for j = 1, mask:size(3) do
		if (mask[{ 1,i,j }] >= 0.95) then
			local left = torch.round(MASK_BOX[{ ind,2,i,j }]*IMW)
			local right = torch.round(MASK_BOX[{ ind,3,i,j }]*IMW)
			local top = torch.round(MASK_BOX[{ ind,4,i,j }]*IMH)
			local bottom = torch.round(MASK_BOX[{ ind,5,i,j }]*IMH)
